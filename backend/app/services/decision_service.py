...
4. speak_prompt도 마찬가지로 두 선택지가 동일한 구조여야 한다.
...
def _imagine_prompt(label: str) -> str:
    return f"'{label}'을(를) 선택한 6개월 뒤의 당신을 상상해보세요."


def _speak_prompt(label: str) -> str:
    return f"'{label}'을(를) 떠올렸을 때 드는 생각을 편하게 말해주세요."
