...
def _record(
    ...
    *,
    ...
    storage.put(
        "decisions", decision.id, decision.model_dump(mode="json"), user_id=owner
    )
    storage.put(
        "sessions",
        session.id,
...
def test_pattern_counts_axis_sides_not_labels() -> None:
    ...
    axis = build_value_map("u1").axes[0]
...
def test_split_axis_is_reported_as_split() -> None:
    ...
    pattern = build_value_map("u1").axes[0].lean_pattern
...
def test_all_unlabelled_says_so_rather_than_insufficient() -> None:
    ...
    assert "나누지 못했" in build_value_map("u1").axes[0].lean_pattern
...
def test_pattern_uses_self_statement_not_our_verdict() -> None:
    ...
    session = Session(id="s1", user_id="u1", decision_id="d1")
    storage.put("decisions", "d1", decision.model_dump(mode="json"), user_id="u1")
    storage.put(
        "sessions", "s1", session.model_dump(mode="json"), decision_id="d1", user_id="u1"
    )

    assert build_value_map("u1").axes[0].lean_pattern == "아직 기록이 부족해요."
...
def test_conviction_contrasts_followed_and_diverged() -> None:
    ...
    conviction = build_conviction("u1")
...
def test_conviction_waits_for_enough_records() -> None:
    ...
    _record("1", axis="안정 vs 성장", labels=("남는다", "옮긴다"),
            sides=("안정", "성장"), self_lean=1, retro=(1, 5))
    assert "쌓이면" in build_conviction("u1").note
...
def test_conviction_ignores_retrospectives_without_self_statement() -> None:
    ...
    session = Session(id="s1", user_id="u1", decision_id="d1")  # annotation 없음
    storage.put("decisions", "d1", decision.model_dump(mode="json"), user_id="u1")
    storage.put(
        "sessions", "s1", session.model_dump(mode="json"), decision_id="d1", user_id="u1"
    )
...
def test_empty_history_is_safe() -> None:
    assert build_value_map("u1").axes == []
    assert build_conviction("u1").total_retrospectives == 0
...
def test_value_map_does_not_mix_users() -> None:
    """★남의 기록이 내 가치관 지도에 들어오면 안 된다.

    user_id 가 생기기 전에는 모든 세션이 한 사람 것처럼 집계됐다. 사용자가 둘만
    돼도 지도가 거짓이 되고, 프라이버시 사고이기도 하다.
    """
    _record("1", axis="안정 vs 성장", labels=("남는다", "옮긴다"),
            sides=("안정", "성장"), self_lean=1, owner="u1")
    _record("2", axis="안정 vs 성장", labels=("남는다", "옮긴다"),
            sides=("안정", "성장"), self_lean=0, owner="u2")
    _record("3", axis="관계 vs 자율", labels=("모인다", "쉰다"),
            sides=("관계", "자율"), self_lean=0, owner="u2")

    mine = build_value_map("u1")
    assert [a.axis for a in mine.axes] == ["안정 vs 성장"]
    assert mine.axes[0].side_counts == {"성장": 1}

    theirs = build_value_map("u2")
    assert sorted(a.axis for a in theirs.axes) == ["관계 vs 자율", "안정 vs 성장"]
    assert theirs.axes[1].side_counts == {"안정": 1} or theirs.axes[0].side_counts == {"관계": 1}


def test_conviction_does_not_mix_users() -> None:
    """남의 만족도가 내 확신의 근거가 되면 안 된다."""
    _record("1", axis="안정 vs 성장", labels=("남는다", "옮긴다"),
            sides=("안정", "성장"), self_lean=1, retro=(1, 5), owner="u1")
    _record("2", axis="안정 vs 성장", labels=("남는다", "옮긴다"),
            sides=("안정", "성장"), self_lean=1, retro=(0, 1), owner="u2")
    _record("3", axis="관계 vs 자율", labels=("모인다", "쉰다"),
            sides=("관계", "자율"), self_lean=0, retro=(0, 5), owner="u2")

    assert build_conviction("u1").total_retrospectives == 1
    assert build_conviction("u2").total_retrospectives == 2


def test_unknown_user_sees_nothing() -> None:
    """처음 온 사람에게 남의 기록을 보여주면 안 된다."""
    _record("1", axis="안정 vs 성장", labels=("남는다", "옮긴다"),
            sides=("안정", "성장"), self_lean=1, owner="u1")
    assert build_value_map("stranger").axes == []
    assert build_conviction("stranger").total_retrospectives == 0
