...
src/
  ...
  lib/
    ...
    faceRoi.ts           ★MediaPipe 얼굴 ROI → RGB 평균 시계열
...
**영상은 브라우저를 떠나지 않는다.** `faceRoi.ts`가 얼굴 ROI의 평균 R/G/B 숫자만
뽑아 보낸다. 프레임이나 인코딩된 영상을 서버로 보내는 코드를 추가하지 마라 —
프라이버시는 기능이 아니라 전제다.
...
언마운트되면서 `srcObject` 연결과 ROI 샘플러 바인딩이 끊긴다.
